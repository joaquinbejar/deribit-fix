...
async fn test_business_message_reject_unsupported_message_type() -> Result<()> {
        ...
        match timeout(Duration::from_millis(500), client.receive_message()).await {
            Ok(Ok(Some(message))) => {
                if let Some(msg_type) = message.get_field(35)
                    && msg_type == "j" { // BusinessMessageReject
    ...
    info!("📊 Test results:");
    info!("  - BusinessMessageReject received: {}", if business_reject_received { "✅ Yes" } else { "ℹ️  No (test env)" });
...
async fn test_business_message_reject_invalid_application_request() -> Result<()> {
    ...
    info!("  - BusinessMessageReject received: {}", if business_reject_received { "✅ Yes" } else { "ℹ️  No (test env)" });
    info!("  - Appropriate reject reason: {}", if invalid_request_reject { "✅ Yes" } else { "ℹ️  N/A" });
...
async fn test_business_message_reject_application_not_available() -> Result<()> {
                ...
                if let Some(msg_type) = message.get_field(35) {
                    match msg_type.as_str() {
                        "j" => { // BusinessMessageReject
                            ...
                            info!("📤 Received BusinessMessageReject #{}", business_reject_count);
    ...
    info!("  - Application not available: {}", if app_not_available { "⚠️  Yes" } else { "✅ No (app available)" });
...
async fn test_business_message_reject_comprehensive_scenarios() -> Result<()> {
                ...
                if let Some(msg_type) = message.get_field(35) {
                    match msg_type.as_str() {
                        "j" => { // BusinessMessageReject
                            ...
                            info!("📤 Received BusinessMessageReject #{}", total_business_rejects);
    ...
    info!("  - Total BusinessMessageReject messages: {}", total_business_rejects);
    info!("  - Business reject reasons encountered: {:?}", business_reject_reasons);
