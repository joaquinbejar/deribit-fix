...
mod tests {
    ...
    fn test_connection_url_generation() {
        let config = DeribitFixConfig::new().with_endpoint("test.example.com".to_string(), 9881);
    ...
    fn test_connection_timeout_config() {
        let config = DeribitFixConfig::new().with_connection_timeout(Duration::from_secs(30));

        assert_eq!(config.connection_timeout, Duration::from_secs(30));

        // Test with different timeout
        let config_fast =
            DeribitFixConfig::new().with_connection_timeout(Duration::from_millis(500));
    ...
    fn test_heartbeat_interval_config() {
        let config = DeribitFixConfig::new().with_heartbeat_interval(60);

        assert_eq!(config.heartbeat_interval, 60);

        // Test with different interval
        let config_fast = DeribitFixConfig::new().with_heartbeat_interval(15);
    ...
    fn test_connection_error_scenarios() {
        ...
        let connection_refused =
            DeribitFixError::Connection("Connection refused by server".to_string());
        assert!(format!("{connection_refused}").contains("Connection error"));

        let timeout_error =
            DeribitFixError::Timeout("Connection timeout after 10 seconds".to_string());
        ...
        let protocol_error = DeribitFixError::Protocol("Invalid FIX message format".to_string());
    ...
    fn test_message_parsing_scenarios() {
        ...
        let msg_error =
            DeribitFixError::MessageConstruction("Failed to construct FIX message".to_string());
