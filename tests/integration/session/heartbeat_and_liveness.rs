...
async fn test_heartbeat_response_to_test_request() -> Result<()> {
    ...
    config.validate()?;
    info!("✅ Configuration loaded with heartbeat interval: {}s", config.heartbeat_interval);
    ...
    if let Some(state) = final_state {
        assert_eq!(state, SessionState::LoggedOn, "Session should remain logged on throughout the test");
    } else {
    ...
    info!("  - Heartbeat messages received: {}", if heartbeat_received { "✅ Yes" } else { "❌ No" });
    info!("  - TestRequest messages received: {}", if test_request_received { "✅ Yes" } else { "❌ No" });
...
async fn test_session_liveness_with_inactivity() -> Result<()> {
    ...
    info!("  - Messages received during inactivity: {}", messages_received);
    info!("  - Session stayed active: {}", if session_stayed_active { "✅ Yes" } else { "❌ No" });
    
    // The session should remain active even during periods of inactivity
    assert!(session_stayed_active, "Session should remain active during inactivity periods");
...
async fn test_heartbeat_interval_configuration() -> Result<()> {
    ...
    for interval in test_intervals {
        ...
        match client.connect().await {
            Ok(_) => {
                info!("✅ Successfully connected with {}s heartbeat interval", interval);
                ...
                for _ in 0..10 {
                    if let Ok(Ok(Some(_))) = timeout(Duration::from_millis(200), client.receive_message()).await {
                ...
                if logon_confirmed {
                    ...
                    warn!("⚠️ No logon confirmation received for {}s interval", interval);
                }
                
                client.disconnect().await.ok();
            }
            Err(e) => {
                warn!("⚠️ Failed to connect with {}s heartbeat interval: {}", interval, e);
