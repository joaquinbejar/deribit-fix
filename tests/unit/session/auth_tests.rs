...
mod tests {
    ...
    fn test_nonce_format_and_length() {
        ...
        for _ in 0..10 {
            ...
            assert_eq!(parts.len(), 2, "RawData should contain exactly one period separator");
            ...
            assert!(nonce_bytes.len() >= 32, 
                "Nonce should be at least 32 bytes, got {} bytes", nonce_bytes.len());
    ...
    fn test_password_hash_uniqueness() {
        ...
        assert_eq!(password_hashes.len(), 10, 
    ...
    fn test_raw_data_format_compliance() {
        ...
        assert!(raw_data.contains('.'), "RawData should contain period separator");
        ...
        assert!(nonce_part.chars().all(|c| c.is_ascii_alphanumeric() || c == '/' || c == '+' || c == '='), 
            "Nonce part should be valid base64");
        
        // Test that nonce decodes to at least 32 bytes
        let nonce_bytes = BASE64_STANDARD.decode(nonce_part)
            .expect("Nonce should be valid base64");
        assert!(nonce_bytes.len() >= 32, 
    ...
    fn test_different_access_secrets() {
        ...
        if raw_data1 == raw_data2 {
            assert_ne!(password_hash1, password_hash2, 
    ...
    fn test_deribit_spec_compliance() {
        ...
        let parts: Vec<&str> = raw_data.split('.').collect();
        assert_eq!(parts.len(), 2, "RawData should be in format: timestamp.nonce");
        ...
        assert!(timestamp <= now_ms && timestamp >= now_ms - 1000, 
