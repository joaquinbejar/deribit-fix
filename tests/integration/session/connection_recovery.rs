...
async fn test_basic_connection_recovery() -> Result<()> {
    ...
    if logon_result.is_err() {
        ...
        return Err(DeribitFixError::Timeout("Initial logon timeout".to_string()));
    }
    ...
    info!("📊 Initial session activity: {} messages received", messages_received);
            ...
            match recovery_confirmed {
                Ok(_) => {
                    ...
                    while test_start.elapsed() < test_duration {
                        match timeout(Duration::from_millis(200), recovery_client.receive_message()).await {
                            Ok(Ok(Some(_))) => {
                                recovery_messages += 1;
                            }
                            _ => {
    ...
    for cycle in 1..=connection_cycles {
        ...
        match connect_result {
            Ok(_) => {
                ...
                while start_time.elapsed() < activity_duration {
                    if let Ok(Ok(Some(_))) = timeout(Duration::from_millis(100), client.receive_message()).await {
                        cycle_messages += 1;
                    } else {
            ...
            match client2.connect().await {
                Ok(_) => {
                    ...
                    while recovery_start.elapsed() < recovery_timeout {
                        ...
                        if let Ok(Ok(Some(_message))) = timeout(Duration::from_millis(100), client2.receive_message()).await {
...
async fn test_rapid_connect_disconnect_cycles() -> Result<()> {
    ...
    info!("  - Successful connections: {}/{}", successful_connections, rapid_cycles);
    ...
    if success_rate >= 0.6 { // 60% success rate is reasonable for rapid cycles
