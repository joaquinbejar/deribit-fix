                ...
                if let Some(seq_num_str) = message.get_field(34)
                    && let Ok(seq_num) = seq_num_str.parse::<u32>() {
                        ...
                        if incoming_seq_nums.len() > 1 {
                            ...
                            if seq_num <= prev_seq {
                                warn!("⚠️ Potential sequence issue: current={}, previous={}", seq_num, prev_seq);
                                sequence_issues += 1;
                            }
                        }
                    }
...
async fn test_session_recovery_after_sequence_gap() -> Result<()> {
    ...
    info!("  - Session remained active: {}", if session_remained_active { "✅ Yes" } else { "❌ No" });
    
    // Session should remain active even if sequence recovery occurs
    assert!(session_remained_active, "Session should remain active during sequence recovery");
