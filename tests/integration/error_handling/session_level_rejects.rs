...
async fn test_reject_message_with_missing_required_tag() -> Result<()> {
    ...
    info!("📊 Test results:");
    info!("  - Reject message received: {}", if reject_received { "✅ Yes" } else { "❌ No" });
    ...
    if let Some(reason) = session_reject_reason {
        ...
        if reason == "1" {
            ...
            warn!("⚠️  Unexpected SessionRejectReason: expected '1', got '{}'", reason);
        }
    }
...
async fn test_reject_message_with_undefined_tag() -> Result<()> {
    ...
    info!("  - Reject received: {}", if reject_received { "✅ Yes" } else { "ℹ️  No (test env)" });
    info!("  - Correct reject reason: {}", if undefined_tag_reject { "✅ Yes" } else { "ℹ️  N/A" });
...
async fn test_reject_message_with_invalid_tag_value() -> Result<()> {
    ...
    info!("  - Reject received: {}", if reject_received { "✅ Yes" } else { "ℹ️  No (test env)" });
    info!("  - Correct reject reason: {}", if invalid_format_reject { "✅ Yes" } else { "ℹ️  N/A" });
