...
async fn test_ssl_connection_establishment() -> Result<()> {
    ...
    config.validate()?;
    info!(
        "✅ SSL configuration created - Host: {}, Port: {}, SSL: {}",
        ...
        Ok(_) => {
            ...
            match logon_result {
                Ok(_) => {
                    ...
                    info!(
                        "📊 SSL session activity: {} messages received",
                        messages_received
                    );
        ...
        Err(e) => {
            ...
            match &e {
                DeribitFixError::Connection(msg) if msg.contains("TLS") => {
                    info!(
                        "ℹ️ SSL connection failed as expected in test environment: {}",
                        msg
                    );
...
async fn test_ssl_configuration_validation() -> Result<()> {
    ...
    for (description, use_ssl, test_mode) in test_scenarios {
        ...
        match config.validate() {
            ...
            Err(e) => {
                warn!(
                    "❌ Configuration invalid for scenario {}: {}",
                    description, e
                );
            }
        }
...
async fn test_ssl_environment_variables() -> Result<()> {
    ...
    for (description, ssl_value, port_value) in test_cases {
        ...
        if config.use_ssl == expected_ssl {
            info!(
                "✅ SSL setting correctly read from environment: {}",
                config.use_ssl
            );
        } else {
            warn!(
                "❌ SSL setting mismatch: expected {}, got {}",
                expected_ssl, config.use_ssl
            );
        }

        if let Some(expected_port) = port_value
            && let Ok(port_num) = expected_port.parse::<u16>()
        {
...
async fn test_ssl_error_handling() -> Result<()> {
    ...
    match ssl_result {
        ...
        Err(other_err) => {
            info!(
                "ℹ️ SSL connection failed with different error: {}",
                other_err
            );
        }
        Ok(_) => {
    ...
    match non_ssl_result {
        Err(e) => {
            info!(
                "✅ Non-SSL connection to SSL port failed as expected: {}",
                e
            );
        }
        Ok(_) => {
